 ...
 * 
 * Based on WordPress GitHub Plugin Updater by Joachim Kudish
...
/**
 * WordPress GitHub Plugin Updater Class
 */
class GitHub_Updater {
    ...
    public function __construct( $config = array() ) {
        ...
        // if the minimum config isn't set, issue a warning and bail out of the updater
    ...
    public function get_new_version() {
        ...
        if ( $this->overrule_transients() || ( !isset( $version ) || !$version || '' == $version ) ) {
            ...
            $raw_response = $this->remote_get( trailingslashit( $this->config['raw_url'] ) . basename( $this->config['readme'] ) );
            ...
            if (is_array($raw_response)) {
                if (!empty($raw_response['body']))
                    $version = $this->get_version_from_file($raw_response['body']);
            } else {
                $version = $this->get_version_from_file($raw_response);
            }
     ...
     * @return string|bool
     */
    private function get_version_from_file( $contents ) {
        // Try to get version from plugin header
        if ( preg_match( '/^Version:\s*(.+)$/mi', $contents, $matches ) ) {
            return trim( $matches[1] );
        }

        // Try to get version from ~Current Version: pattern
        if ( preg_match( '/~Current Version:\s*([^~]+)~/i', $contents, $matches ) ) {
            return trim( $matches[1] );
        }
